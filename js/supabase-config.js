...
var invites = {
    ...
    inviteToRoom: async function(roomId, rawInput) {
          ...
          for (var i = 0; i < recipients.length; i++) {
            ...
            var existing = await _supabase
                    ...
                    .maybeSingle();
                  // Guard: surface DB errors instead of crashing on existing.data access
            if (existing.error) {
                      results.push({ raw: rec.raw, email: resolved.email, token: null, error: existing.error.message });
                      continue;
            }
                  if (existing.data) {
    ...
    acceptInvite: async function(token) {
          ...
          if (r.data.status === 'accepted') return { roomId: r.data.room_id, error: null };
